...
public class Jarnbox {	
	...
	private JButton bb(String action){
    	...
    	private JCheckBox bc(String label, String action){
        	JCheckBox item;
        	item = new JCheckBox(Jarnal.trans(label));
	...
	public void showCancel(){
		...
		c.gridy = 2;
		cp.add(bb("Cancel"), c);
            	jw.setSize(new Dimension(200, 100));
		setCenter(jw);
	...
	public int showSaveExitCancel(){
		...
		jw.getContentPane().add(new JLabel(Jarnal.trans("Unsaved data will be lost. OK to exit?")));
		JPanel bot = new JPanel();
		bot.add(bb("Save"));
		bot.add(bb("Cancel"));
		bot.add(bb("Exit Without Saving"));
		jw.getContentPane().add(bot);
		jw.setSize(new Dimension(450, 120));
		setCenter(jw);
		jw.addWindowListener(new dialogClosing(new dialogListener("Cancel", this)));
		jw.setVisible(true);
		while(!done){}
	...
	public void showUnRe(){
		...
		cp.add(bb("Undo/Redo"), c);
            	jw.setSize(new Dimension(300, 300));
		setCenter(jw);
		jw.addWindowListener(new dialogClosing(new dialogListener("Cancel", this)));
            	jw.setVisible(true);
	}
	...
	public void showFind(){
		...
		c.gridx = 4;
		cp.add(bb("Find"), c);		
            	jw.setSize(new Dimension(500, 150));
		setCenter(jw);
		jw.addWindowListener(new dialogClosing(new dialogListener("Cancel", this)));
            	jw.setVisible(true);
	}
	...
	public void showLink(String link){
		...
		cp.add(bb("Insert Link"), c);		
            	jw.setSize(new Dimension(400, 100));
		setCenter(jw);
		jw.addWindowListener(new dialogClosing(new dialogListener("Cancel", this)));
            	jw.setVisible(true);
	}		
	...
	public void showReplay(){
		...
		c.gridx = 4;
		cp.add(bb("Close"), c);
            	jw.setSize(new Dimension(350, 100));
		setCenter(jw);
	...
	public Number getPosDecNumber(double init, double max, boolean redrawButton){
	    ...
	    if(redrawButton) width = 250;
            jw.setSize(new Dimension(width, 100));
	    setCenter(jw);
	    jw.addWindowListener(new dialogClosing(new dialogListener("Cancel", this)));
            jw.setVisible(true);
	    if(!redrawButton){
	...
	public Number getInt(int init, int min, int max, boolean redrawButton, boolean saveBg){
	    ...
	    if(saveBg) height = 150;
            jw.setSize(new Dimension(width, height));
	    setCenter(jw);
	    jw.addWindowListener(new dialogClosing(new dialogListener("Cancel", this)));
            jw.setVisible(true);
	    if(!redrawButton){
	...
	public String getString(String init){
		...
		if(jarn.pencentric) {
			...
			cp.add(jpan, c);
			jw.setSize(new Dimension(600, 330));
		}					
            	else jw.setSize(new Dimension(400, 100));
		setCenter(jw);
		jw.addWindowListener(new dialogClosing(new dialogListener("Cancel", this)));
            	jw.setVisible(true);
	    	while(!done){}
...
class dialogListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e){
		...
		if(action.equals("Exit Without Saving")){
			jb.getDialog().setVisible(false);
			jb.buttonInt = 2;
			jb.done = true;
		}
...
class paperDialogListener implements ActionListener {
	...
	public void showDialog(JFrame jf, Jarnal jarn){
		...
		c.gridy++;
		setRuling();
            	jw.setSize(new Dimension(400, 570));
...
class checkForUpdatesListener implements ActionListener {
	...
	private void checkVersions(){
		HtmlPost hp = new HtmlPost(jarn.homeserver + "jarnal-ver.txt", null, null, null, null, false);
		byte b[] = hp.pipeBytes();
		if(b != null) curver = new String(b, 0, b.length);
		hp = new HtmlPost(jarn.homeserver + "stable-jarnal-ver.txt", null, null, null, null, false);
		b = hp.pipeBytes();
		if(b != null) stableverstr = new String(b, 0, b.length);
		hp = new HtmlPost(jarn.homeserver + "/lib/lib-ver.txt", null, null, null, null, false);
		b = hp.pipeBytes();
	...
	private void getReleaseNotes(){
		...
		est = Jtool.replaceAll(est, "%1", jarn.homeserver + "../../../workshops/changelog.php?econsite=econsjarn");
		try{
	...
	private void showDialog(JFrame jf, Jarnal jarn, String action){
		...
		item.addActionListener(this);
		cp.add(item, c);
            	jw.setSize(new Dimension(380, 260));
		Jarnbox.setCenter(jf, jw);
            	jw.setVisible(true);
	...
	private void downloadFile(String action){
		...
		HtmlPost hp = new HtmlPost(jarn.homeserver + source, null, null, null, null, false);
...
class printOptionsListener implements ActionListener {
	...
	public void showDialog(JFrame jf, Jarnal jarn){
		...
		cp.add(item, c);
            	...
            	jw.setSize(new Dimension(240, 260));
		Jarnbox.setCenter(jf, jw);
            	jw.setVisible(true);
		jw.addWindowListener(new dialogClosing(this, "Exit This Dialog"));
	}
...
class overlayDialogListener implements ActionListener {
	...
	public void showDialog(JFrame jf, Jarnal jarn){
		...
		gridBagConstraints.gridwidth = 3;
		gridBagConstraints.fill = java.awt.GridBagConstraints.HORIZONTAL;
		cp.add(separator, gridBagConstraints);
		...
		button = new JButton(Jarnal.trans("Refresh"));
		button.setActionCommand(action);
		button.addActionListener(this);
		gridBagConstraints = new java.awt.GridBagConstraints();
		gridBagConstraints.gridx = 0;
...
class textDialogListener implements ActionListener {
	...
	public void showDialog(JFrame jf, Jarnal jarn){
		...
		cp.add(item, c);
		setDialog();		
            	jw.setSize(new Dimension(240, 340));
		Jarnbox.setCenter(jf, jw);
            	jw.setVisible(true);
		jw.addWindowListener(new dialogClosing(this, "Done"));
	}
